...
export type PortalImageCardProps = {
  href: string;
  image: string;
  imageAlt: string;
  overlay: string;
...
export function PortalImageCard({
  href,
  image,
  imageAlt,
  overlay,
  ...
  index,
  priority = false,
}: PortalImageCardProps) {
  const reduce = useReducedMotion();

  const card = (
    <Link
      href={href}
      className="portal-card group relative flex aspect-[3/4] min-h-[280px] w-full flex-col overflow-hidden rounded-[1.75rem] sm:min-h-[300px] sm:rounded-[2rem] lg:aspect-[4/5] lg:min-h-[320px]"
    ...
    >
      ...
      <div
        className={`portal-card-overlay pointer-events-none absolute inset-0 bg-gradient-to-t ${overlay}`}
        aria-hidden
      />
      <div
      ...
      <span
        className="portal-card-plus absolute right-3 top-3 z-10 flex h-9 w-9 items-center justify-center rounded-full text-lg font-light backdrop-blur-md transition-all duration-300 group-hover:scale-110 group-hover:shadow-glow-teal sm:right-4 sm:top-4 sm:h-10 sm:w-10"
        aria-hidden
      >
        +
      </span>
      ...
      <div className="portal-card-glass relative z-10 mt-auto p-3 sm:p-4">
        <div className="flex items-start gap-2.5">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-2 line-clamp-3 text-xs leading-relaxed text-white/72 sm:text-sm">
              {description}
            </p>
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.75, delay: index * 0.06, ease: easeEditorial }}
    >
      {card}
    </motion.div>
  );
